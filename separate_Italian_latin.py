...
from dict import replace_abbreviations
from matchdate import extract_latin_date
...
INPUT_FILE = os.path.join(DATA_DIR, "Giovanni_scriba_1_HYPHEN_CLEANED.txt")    
OUTPUT_ITALIAN = os.path.join(DATA_DIR, "output_italian.txt")
OUTPUT_LATIN = os.path.join(DATA_DIR, "output_latin.txt")
OUTPUT_NONE = os.path.join(DATA_DIR, "output_none.txt")
...
def clean_text(text):
    ...
    for line in text.splitlines():
        ...
        cleaned_line = replace_abbreviations(line.strip())
        cleaned_lines.append(cleaned_line.strip())

    paragraph = "\n".join(cleaned_lines) 
    latin_date = extract_latin_date(paragraph)
    return paragraph, latin_date
...
def separate_italian_latin(input_file, output_italian, output_latin, output_none):
    ...
    with open(input_file, 'r', encoding='utf-8', errors='ignore') as file:
        ...
        for paragraph in paragraphs:
            ...
            paragraph, latin_date = clean_text(paragraph)
            latin_paragraphs.append(latin_date)  
